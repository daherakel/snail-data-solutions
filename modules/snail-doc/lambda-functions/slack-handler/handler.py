...
QUERY_HANDLER_FUNCTION = os.environ.get('QUERY_HANDLER_FUNCTION', f'{PROJECT_NAME}-{ENVIRONMENT}-query-handler')
VERIFY_SLACK_SIGNATURE = os.environ.get('VERIFY_SLACK_SIGNATURE', 'true').lower() == 'true'
...
CONVERSATIONS_TABLE_NAME = os.environ.get('CONVERSATIONS_TABLE_NAME', f'{PROJECT_NAME}-{ENVIRONMENT}-conversations')

# Clientes AWS
lambda_client = boto3.client('lambda', region_name=AWS_REGION)
dynamodb_client = boto3.client('dynamodb', region_name=AWS_REGION) if CONVERSATIONS_TABLE_NAME else None
...
def get_slack_secrets() -> tuple[str, str]:
    ...
    try:
        ...
        if bot_token_secret_arn:
            ...
            bot_token = secret_data.get('bot_token') or secret_data.get('SLACK_BOT_TOKEN') or secret_data.get('token')
        
        if signing_secret_arn:
            response = secrets_client.get_secret_value(SecretId=signing_secret_arn)
            secret_data = json.loads(response['SecretString'])
            signing_secret = secret_data.get('signing_secret') or secret_data.get('SLACK_SIGNING_SECRET') or secret_data.get('secret')
    ...
    if not SLACK_BOT_TOKEN or not SLACK_SIGNING_SECRET:
        raise ValueError("SLACK_BOT_TOKEN y SLACK_SIGNING_SECRET deben estar configurados")
...
def send_slack_message(channel: str, text: str, thread_ts: Optional[str] = None) -> Dict[str, Any]:
    """
...
def load_conversation_history(conversation_id: str, limit: int = 10) -> list[Dict[str, str]]:
    """
...
def process_query(query: str, conversation_id: Optional[str] = None, conversation_history: Optional[list] = None) -> Dict[str, Any]:
    """
