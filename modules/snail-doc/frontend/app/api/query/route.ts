...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      // Pasar el error del Lambda al frontend
      return NextResponse.json(
        { 
          error: errorData.error || 'Failed to process request', 
          details: errorData.detail || errorData.details || errorData.error
        },
    ...
    // Si el Lambda devuelve un error en el body (status 200 pero con error)
    if (data.error) {
      return NextResponse.json(
        { error: data.error, details: data.detail },
        { status: 500 }
      );
    }
